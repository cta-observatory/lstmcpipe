...
for col_hipe in [c for c in selected_events.columns if 'hipecta_' in c]:
    ...
    if np.count_nonzero(both_finite) > 1:
        axes[0].hist(selected_events[col_lst][both_finite], **opt, label='lstchain')
        axes[0].hist(selected_events[col_hipe][both_finite], **opt, label='hipecta')
        axes[0].set_xlabel(feature_name, fontsize=16)
        axes[0].legend(fontsize=16)
        axes[0].grid()

        axes[1].hist2d(selected_events[col_lst][both_finite], selected_events[col_hipe][both_finite], bins=100)
