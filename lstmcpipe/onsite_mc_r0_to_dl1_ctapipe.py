...
def main(input_dir, config_file=None, train_test_ratio=0.5, random_seed=42, n_r0_files_per_dl1_job=0,
    ...
    if source_environment is not None:
        manage_source_env_r0_dl1(source_and_env=source_environment, file=os.path.abspath('./core_list_ctapipe.sh'))
    ...
    for set_type in 'training', 'testing':
        ...
        for file in os.listdir(dir_lists):
            ...
            output = Path(output_dir) / (file.split('.')[0] + '.dl1.h5')
            base_cmd = f'core_list_ctapipe.sh "ctapipe-stage1 --write-images --write-parameters {cc}"'
