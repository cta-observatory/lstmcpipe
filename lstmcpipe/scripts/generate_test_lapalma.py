...
for particle in tqdm(['gamma-diffuse', 'proton', 'electron', 'gamma']):
    for i, file in enumerate(
            aswg_base_dir.glob(f'DL0/20200629_prod5_trans_80/{particle}/zenith_20deg/south_pointing/**/*.simtel.gz')):
        if i >= nfiles:
            break
        else:
            target = lstmcpipe_base_dir.joinpath(file.relative_to(aswg_base_dir))
